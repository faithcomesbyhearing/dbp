...
class SeedBibleStrongs extends Seeder
{
    ...
    private function seedLexicalDefinitions($lex, $word_letter)
    {
        ...
        foreach (collect($lex->data->def->long) as $definition) {
            if (is_array($definition)) {
                $to_skip = $this->processDefinitionArray($definition, $word_letter, $lex, $to_skip);
            } else {
                LexicalDefinition::create([
                    'lexicon_id' => $word_letter.$lex->strongs,
    ...
    private function processDefinitionArray($definition, $word_letter, $lex, $to_skip)
    {
        foreach ($definition as $key => $word) {
            ...
            if (is_array($next)) {
                foreach ($next as $sub_word) {
                    $sub_word = $this->normalizeSubWord($sub_word);
    ...
    private function normalizeSubWord($sub_word)
    {
        ...
        return collect($sub_word)->flatten();
    }
}
